...
bool check_P1_2vec_3d()
{
    ...
    FEM_Matrix_2<P1_2vec_3d> SK(mesh, elem);           // allocate matrix
    //int const nP1 = elem.getP1_nodes();
    //int const nP2 = elem.getP2_nodes();
    vector<double> uv(SK.Nrows(), 0.0);    //          temperature
    vector<double> fv(SK.Nrows(), 0.0);    //          r.h.s.
    ...
    vector<double> uv_bc(SK.Nrows(), 1.0);
    ...
    SK.SetValues(uv_ic, [](double x, double y, double z) -> double {return 1;}, // fs     //x*y*z+x*x*x*y*y*y*z-5.0/32
                        [](double x, double y, double z) -> double {return sin(z)+cos(y);}, // fx   cos(x) *sin(y) *sin(z)    //x+x*x+x*y+x*x*x*y
                        [](double x, double y, double z) -> double {return sin(x)+cos(z);}, // fy    -sin(x) *cos(y) *sin(z)   //y+x*y+y*y+x*x*y*y
                        [](double x, double y, double z) -> double {return sin(y)+cos(x);} ); // fz   //-2*z-3*x*z-3*y*z-5*x*x*y*z
    ...
    for (int ni = 0; ni < t_end / dt; ++ni) {
        ...
        SK.SetValues(uv_ex, [t_ni](double x, double y, double z) -> double {return 1;}, // fs     //x*y*z+x*x*x*y*y*y*z-5.0/32
                            [mu,t_ni](double x, double y, double z) -> double {return (sin(z)+cos(y))*exp(-mu*t_ni);}, // fx   cos(x) *sin(y) *sin(z)    //x+x*x+x*y+x*x*x*y
                            [mu,t_ni](double x, double y, double z) -> double {return (sin(x)+cos(z))*exp(-mu*t_ni);}, // fy    -sin(x) *cos(y) *sin(z)   //y+x*y+y*y+x*x*y*y
                            [mu,t_ni](double x, double y, double z) -> double {return (sin(y)+cos(x))*exp(-mu*t_ni);} ); // fz   //-2*z-3*x*z-3*y*z-5*x*x*y*z
        ...
        for (int mj = 0; mj < 5; ++mj) {
            ...
            SK.Navier_Stokes(fv, r_old_n, r_old_m, u_old_n, u_old_m, v_old_n, v_old_m, w_old_n, w_old_m, dt, mu, lambda, kp, t_ni);     // Fill stiffness matrix and rhs ()
            //cout << "fv= " <<fv << endl;
            //SK.CalculateLaplace(fv);
            //SK.Debug();
