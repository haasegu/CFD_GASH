 ...
 */
void CalcElem_heat_equation_crank_nichelson(int const ial[4], double const xc[], std::vector<std::vector<double>> &ske, std::vector<double> &fe, const std::vector<double> &u_old, 
...
class FEM_Matrix_2: public CRS_Matrix
{
    ...
    { return _elem.nDim_loc(); }  
       ...
       */
       ...
       void Navier_Stokes(
           std::vector<double> &f, 
           const std::vector<double> &r_old_n, const std::vector<double> &r_old_m, const std::vector<double> &u_old_n, const std::vector<double> &u_old_m, const std::vector<double> &v_old_n, const std::vector<double> &v_old_m, const std::vector<double> &w_old_n, const std::vector<double> &w_old_m, 
    const double dt, const double mu, const double lambda, const double kp, const double t_ni);
     ...
     void Write_ascii_paraview_r(std::string const &fname, std::vector<double> const &r) const;
     
     void Visualize_paraview_r(std::vector<double> const &r) const;
        ...
        const std::function<double(double, double, double)> &fz) const
{
    ...
    {
		int kv = nP1+k;
        vvec[kv] = fx( xc[3*k], xc[3*k+1] , xc[3*k+2]);
        int kv1 = nP1+nP2+k;
        vvec[kv1] = fy( xc[3*k], xc[3*k+1] , xc[3*k+2]);
        int kv2 = nP1+2*nP2+k;
        vvec[kv2] = fz( xc[3*k], xc[3*k+1] , xc[3*k+2]);
    }
...
void FEM_Matrix_2<ELEM>::Derive_Matrix_Pattern()
{
    ...
    //std::cout << "Debug _ia_dof" << endl;
    //std::cout << _ia_dof << endl;
...
void FEM_Matrix_2<ELEM>::Navier_Stokes(
    std::vector<double> &f, 
    const std::vector<double> &r_old_n, const std::vector<double> &r_old_m, const std::vector<double> &u_old_n, const std::vector<double> &u_old_m, const std::vector<double> &v_old_n, const std::vector<double> &v_old_m, const std::vector<double> &w_old_n, const std::vector<double> &w_old_m, 
    const double dt, const double mu, const double lambda, const double kp, const double t_ni)  // 3D
    //const std::variant<std::function<double(double, double)>, std::function<double(double, double, double)>> &f_func)
{
    assert(_elem.nDim_loc()==3);                                  // 3D
    std::cout << "\n############   FEM_Matrix::Navier_Stokes f_func 3D\n";
    //std::cout << _nnz << " vs. " << _id[_nrows] << "  " << _nrows<< std::endl;
    assert(_nnz == _id[_nrows]);
...
//    #pragma omp for    
    for (int i = 0; i < nelem; ++i) {             //  Loop over all elements
        _elem.CalcElem_Navier_Stokes(ia_geom.data() + nvert_e * i, xc.data(), ske, fe,r_old_n, r_old_m, u_old_n, u_old_m, v_old_n, v_old_m, w_old_n, w_old_m, dt, mu, lambda, kp, t_ni);
        //std::cout << i << " : " << ia_crs.size() << " : " <<  ndof_e * i << "(" << ndof_e << ") ";
        //cout << "ske : " << ske << endl;
        AddElem(ia_crs.data() + ndof_e * i, ske, fe, f);
        //std::cout << " # \n";
    }
...
void FEM_Matrix_2<ELEM>::Write_ascii_paraview_u(std::string const &fname, std::vector<double> const &u) const
{
    ...
    fout << "CELLS "<< Nelems() << ' ' << Nelems()*11 << endl;
