...
vector<int> mergeColumns(vector<int> const &v1, int col1, vector<int> const &v2, int col2, vector<int> const &v3, int col3, vector<int> const &v4, int col4)
{
    ...
    for (int k=0; k<nrows; ++k)
    {
        ...
        {
            vm.at(k*ncols+d+col1+col2) = v3.at(k*col3+d);  // copy data v3
        }
        for (int d=0; d<col4; ++d)
        {
            vm.at(k*ncols+d+col1+col2+col3) = v4.at(k*col4+d);  // copy data v4
        }
    }
...
*/
vector<int> P2_2d::getGlobalDOFIndices(vector<int> const &ia_geom)  const 
{
    assert(0 == nDOFs_loc() % nVertices_loc());
    int const ndof_v = nDOFs_loc()/nVertices_loc();  // DOFs per vertex
...
vector<int> P1_2vec_2d::getGlobalDOFIndices(vector<int> const &ia_geom_P2)  const
{
    int const nv1(3);              // #vertices tet linear
    int const nv2(6);             // #vertices tet quadratic    
...
vector<int> mergeColumns(vector<int> const &v1, int col1, vector<int> const &v2, int col2, vector<int> const &v3, int col3)
{
    assert( v2.size()%col2 == 0 );
    assert( v1.size()%col1 == 0 );
    assert( v1.size()/col1 == v2.size()/col2 );
    int const nrows = static_cast<int>(v2.size()/col2);
    int const ncols = col1+col2+col3;
    vector<int> vm(nrows*ncols);
...
vector<int> P1_2vec_2d::getGlobalDOFIndices(vector<int> const &ia_geom_P2)  const
{
    ...
    vector<int> ia_dofs(mergeColumns(ia_p1, col1, ia_p2, col2, ia_p3, col3));
...
void P1_2vec_2d::CalcElem_Navier_Stokes(
...
//    }
    ...
    ::CalcElem_Navier_Stokes(ial, xc, ske, fe, r_old_n, r_old_m, u_old_n, u_old_m, v_old_n, v_old_m, w_old_n, w_old_m, dt, mu, lambda, a, gamma, t_ni, kt, cp);
